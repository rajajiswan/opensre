...
class Graph:
    ...
    def add_node(self, node: Node) -> "Graph":
        ...
        if node.node_id in self._nodes:
            ...
            # duplicate nodes are caught early -- keeping this as a warning
            # for now but may switch back to ValueError once the config
            # loader deduplicates on its own.
            logger.warning(
                "Node '%s' already exists in graph '%s'; skipping duplicate.",
                node.node_id,
                self.graph_id,
            )
            return self
